...
# 分类要求
1. **仔细阅读分类手册，特别关注功能完备性和系统可靠性的边界说明**
2. **功能完备性判断关键点**：
   - 先判断：页面上是否有该功能的按钮/入口/菜单项？
   - 如果**完全没有**（设计上就没有，页面上找不到）→ 功能完备性
   - 如果**有但不可用**（存在但出故障/报错/异常）→ 系统可靠性 - 系统运行稳定
3. **常见误判情况**：
   - "功能无法使用"、"功能失效"、"功能报错" → 应归为**系统可靠性**，不是功能完备性
   - "功能不存在"、"完全没有该功能"、"缺少某功能" → 才归为**功能完备性**
4. 分析问题描述，判断其属于哪个一级分类
5. 分类结果必须是以下5个一级分类之一：
   - 功能完备性（仅限功能完全不存在的情况）
   - 信息清晰性
   - 任务高效性
   - 系统可靠性（包含功能存在但不可用的情况）
   - 一致性
6. 给出分类结果、简要的分类原因（50字以内）、以及参照的具体章节
7. 以JSON格式返回结果
